...
class CelebADataset(Dataset):
    def __init__(self, data_dir, mode, transform=None):
        ...
        elif mode == "people_without_glasses":
            self.df = self.df[(self.df["Eyeglasses"] == -1)]
        elif mode == "men_with_glasses":
            self.df = self.df[(self.df["Male"] == 1) & (self.df["Eyeglasses"] == 1)]
        elif mode == "women_with_glasses":
             self.df = self.df[(self.df["Male"] == -1) & (self.df["Eyeglasses"] == 1)]
        elif mode == "women_without_glasses":
             self.df = self.df[(self.df["Male"] == -1) & (self.df["Eyeglasses"] == -1)]
        elif mode == "smiling_men":
...
# Datasets
source_dir = "./dataset/celeba/img_align_celeba"
men_no_glasses_dataset = CelebADataset(
    source_dir, "men_no_glasses", transform=transform
)
men_with_glasses_dataset = CelebADataset(
    source_dir, "men_with_glasses", transform=transform
)
women_with_glasses_dataset = CelebADataset(
    source_dir, "women_with_glasses", transform=transform
)
women_without_glasses_dataset = CelebADataset(
    source_dir, "women_without_glasses", transform=transform
)
people_with_glasses_dataset = CelebADataset(
    source_dir, "people_with_glasses", transform=transform
)
people_without_glasses_dataset = CelebADataset(
   source_dir, "people_without_glasses", transform=transform
)
smiling_men_dataset = CelebADataset(

...
class Encoder(nn.Module):
    def __init__(self, channels, features_gen, noise_dim):
        ...
        # write the encoder nn.sequential with 6 conv layers, leaky relu and batch norm 
        self.encoder = nn.Sequential(
            # layer 1, input img: 64 * 64
            nn.Conv2d(channels, features_gen, kernel_size=4, stride=2, padding=1),
            nn.LeakyReLU(0.2),
            nn.BatchNorm2d(features_gen),
            # layer 2, input img: 32 * 32
            nn.Conv2d(features_gen, features_gen*2, kernel_size=4, stride=2, padding=1),
            ...
            nn.BatchNorm2d(features_gen*16),
            # layer 6, input img: 2 * 2
            nn.Conv2d(features_gen*16, noise_dim, kernel_size=2, stride=1, padding=0),  # Adjusted kernel size
            nn.Flatten()
        )
